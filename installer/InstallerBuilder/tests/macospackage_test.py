...
class TestableMacOSPackageFile(macospackage.MacOSPackageFile):
    def __init__(self, srcDir, targetDir, installerDir, intermediateDir, configuration):
        macospackage.MacOSPackageFile.__init__(self, srcDir, targetDir, installerDir, intermediateDir, configuration)
...
class MacOSPackageFileTestCase(unittest.TestCase):
    ...
    def GivenTestableMacOSPackageFile(self, pfmajor, pfminor_str):
        configuration = {'pf' : 'MacOS',
                         'version' : '<version>',
                         'release' : '<release>',
                         'pfmajor' : pfmajor,
                         'pfminor' : int(pfminor_str),
                         'pfminor_str' : pfminor_str,
                         'pfarch'  : '<pfarch>'}
        return TestableMacOSPackageFile('', '/targetdir/', '', '', configuration)
    ...
    def test10_4PackageFileNameFormat(self):
        ...
        self.assertEqual('/targetdir/scx-<version>-<release>.macos.10.4.<pfarch>.dmg',
                         package.GetPackageFilePath())

    def test10_5PackageFileNameFormat(self):
        package = self.GivenTestableMacOSPackageFile(10, '5')
        self.assertEqual('/targetdir/scx-<version>-<release>.macos.10.5.<pfarch>.pkg',
                         package.GetPackageFilePath())

    def testPreInstallScriptCallsUninstallScriptIfUpgrade(self):
        package = self.GivenTestableMacOSPackageFile(10, '5')
        script = testutils.FakeScript()
        package.PopulatePreInstallScript(script)
        self.assertEqual(
            'if [ -f /usr/libexec/microsoft/scx/bin/scxUninstall.sh ]; then\n' + \
                '  /usr/libexec/microsoft/scx/bin/scxUninstall.sh\n' + \
                'fi\n' + \
                'exit 0\n',
            script.mainSection)
    ...
    def testPreInstallScriptIsInTempDirScripts(self):
        ...
        self.assert_(testutils.FileHasBeenWritten('intermediate/scripts/preinstall'))

    def testPostInstallScriptIsInTempDirScripts(self):
        package = self.GivenTestableMacOSPackageFile(10, '4')
        package.GenerateScripts()
        self.assert_(testutils.FileHasBeenWritten('intermediate/scripts/postinstall'))

    def testGenerateScriptsCreatesTempDirScriptsDirectory(self):
        package = self.GivenTestableMacOSPackageFile(10, '4')
        package.GenerateScripts()
        self.assertEqual('mkdir intermediate/scripts > /dev/null 2>&1', self.TheFakeOS.systemCalls[0])
