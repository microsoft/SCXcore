...
class MacOSPackageFile(Installer):
    ...
    # \param[in] intermediateDir Absolute path to intermediate diretory
    ...
    def __init__(self, srcDir, targetDir, installerDir, intermediateDir, configuration):
        ...
        if self.configuration['pfmajor'] == 10:
            if self.configuration['pfminor'] == 4:
                self.packageHelper = MacOS10_4PackageHelper(self.stagingRootDir,
                                                            self.tempDir,
                                                            self.configuration['version'],
                                                            self.configuration['release'],
                                                            self.scriptDir)
            elif self.configuration['pfminor'] == 5:
                self.packageHelper = MacOS10_5PackageHelper(self.stagingRootDir,
                                                            self.tempDir,
                                                            self.targetDir,
                                                            self.configuration['version'],
                                                            self.configuration['release'],
    ...
    ##
    # Preinstall will uninstall any previously installed version and back up any configuration files
    #
    def PopulatePreInstallScript(self, script):
        script.WriteLn('if [ -f /usr/libexec/microsoft/scx/bin/scxUninstall.sh ]; then')
        # This is an upgrade - Run the uninstall script of the installed version.
        script.WriteLn('  /usr/libexec/microsoft/scx/bin/scxUninstall.sh')
        script.WriteLn('fi')
    ...
    def GenerateScripts(self):
        ...
        self.PopulatePreInstallScript(preinstall)
        ...
        uninstallFile = os.path.join(self.stagingRootDir, "usr/libexec/microsoft/scx/bin/scxUninstall.sh")
        uninstall = Script(uninstallFile, self.configuration)
        # Check for administrator privilege
        uninstall.WriteLn('echo "Microsoft SCX version ' + self.configuration['version'] + '-' + self.configuration['release'] + ' uninstallation script\\n"')
        uninstall.WriteLn('')
        ...
        # First shutdown and unregister everything (essentially the same as preinstall)
        uninstall.WriteLn('')
        uninstall.WriteLn('# Uninstall/unconfigure services')
        uninstall.WriteLn('echo "Uninstalling/unconfigurating services ..."')
        uninstall.CallFunction(uninstall.StopOmiService())
        uninstall.CallFunction(uninstall.RemoveOmiService())
        uninstall.CallFunction(uninstall.RemoveConfigFiles())
        uninstall.CallFunction(uninstall.UnconfigureScxPAM())
        uninstall.CallFunction(uninstall.UnconfigurePAM())
        uninstall.CallFunction(uninstall.RemoveAdditionalFiles())
        uninstall.CallFunction(uninstall.DeleteSoftLinkToSudo())
...
class MacOS10_5PackageHelper:
    ...
    def GenerateIndexXml(self):
        ...
        indexfile.write('    <choice title="SCX Installation Files" id="scx" tooltip="Installs SCX Management Files" ' + \
                            'description="Installs and configures the SCX Management Files, including SCX Core Code and OMI." ' + \
